...
%   
    ...
    switch altitude_state,
        case 1,  % in take-off zone
            delta_t = 1;
            theta_des = 20*pi/180;
            theta_c = theta_des/P.K_theta_DC;
            if h>=P.altitude_take_off_zone,
                altitude_state = 2;
                initialize_integrator = 1;
            else
        ...
        case 2,  % climb zone
            delta_t = 1;
            theta_des = airspeed_with_pitch_hold(Va_c, Va, initialize_integrator, P);
            theta_c = theta_des;
        ...
        case 4, % altitude hold zone
            ...
            theta_des = altitude_hold(h_c, h, initialize_integrator, P);
            theta_c = theta_des;
            if h<=h_c-P.altitude_hold_zone,
                altitude_state = 2;
                initialize_integrator = 1;
...
function delta_e = pitch_hold(theta_c, theta, q, flag, P)
  ...
  if flag==1,
      err_int = 0; 
      err_past   = 0; % error at last sample (d1-delayed by one sample)
  end
 
  % compute the current error
  error = theta_c - theta;
  
  % TODO compute the PID control u_pid
  u_pid = error*P.pitch_kp -q*P.pitch_kd;

  % implement PID control
  delta_e = sat(u_pid, 45*pi/180, -45*pi/180);
