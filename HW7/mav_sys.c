...
#define S_FUNCTION_NAME mav_sys
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
{
    const real_T   *Force  = (const real_T*) ssGetInputPortSignal(S,0);
    const real_T   *Torque  = (const real_T*) ssGetInputPortSignal(S,1);
    real_T        *y  = (real_T *)ssGetOutputPortRealSignal(S,0);
    const real_T   *xC = ssGetContStates(S);
    const int_T   p_width0  = mxGetNumberOfElements(PARAM_DEF0(S));
    const real_T  *mass  = (const real_T *)mxGetData(PARAM_DEF0(S));

    mav_sys_Outputs_wrapper(Force, Torque, y, xC, mass, p_width0);
}
...
/* Function: mdlDerivatives =================================================
  ...
  static void mdlDerivatives(SimStruct *S)
  {
    ...
    mav_sys_Derivatives_wrapper(Force, Torque, y,dx,  xC, mass, p_width0);
}
